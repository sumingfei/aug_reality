...
LEDBlinker::LEDBlinker() {
    ...
    FILE *f = fopen("/sys/class/i2c-adapter/i2c-2/2-0032/engine3_mode", "w");
    if (!f) return;
    fwrite("load\n", 5, 1, f);
    fclose(f);
    f = fopen("/sys/class/i2c-adapter/i2c-2/2-0032/engine3_leds", "w");
    if (!f) return;
    fwrite("000011100\n", 10, 1, f);
    fclose(f);
    f = fopen("/sys/class/i2c-adapter/i2c-2/2-0032/engine3_load", "w");
    if (!f) return;
